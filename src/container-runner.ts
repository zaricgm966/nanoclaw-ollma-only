...
function buildVolumeMounts(
  ...
  fs.mkdirSync(path.join(groupIpcDir, 'host-tools', 'requests'), { recursive: true });
  fs.mkdirSync(path.join(groupIpcDir, 'host-tools', 'results'), { recursive: true });
  mounts.push({
  ...
  return new Promise((resolve) => {
    ...
    container.on('close', (code) => {
      ...
      if (onOutput && hadStreamingOutput) {
        outputChain.then(() => {
          logger.info(
            { group: group.name, duration, newSessionId, code },
            'Container completed after streamed output',
          );
          resolve({
            status: 'success',
            result: null,
